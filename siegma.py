...
def create_rule_file_for_siem(notes_folder, sigma_query_format, sigma_config, config, credentials, query, yj_rule, output, testing=False):
	rule_file = None
	...
	if sigma_query_format in ['es-qs']:
		rule_file = es_qs.create_rule(notes_folder, config_copy, sigma_config, credentials, query, yj_rule, attack, output, os.path.dirname(os.path.realpath(__file__)), logger, testing=testing)
	return rule_file
...
def main():
	try:
		...
		for idx, rule in enumerate(get_all_rule_files(args.rule)):
			...
			out_file_name = create_rule_file_for_siem(args.config.get('notes_folder'), args.config.get('sigma_query_format'), args.sigma_config, args.config.get('settings'), args.config.get('credentials'), query, rule, args.output, testing=args.testing)
